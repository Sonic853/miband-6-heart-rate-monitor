...
class MiBand {
  ...
  constructor(authKey) {
    ...
    /**
     * @type {{
     *  miband1: BluetoothRemoteGATTService,
     *  miband2: BluetoothRemoteGATTService,
     ...
     *  hrControl: BluetoothRemoteGATTCharacteristic,
     *  hrMeasure: BluetoothRemoteGATTCharacteristic,
     *  sensor: BluetoothRemoteGATTCharacteristic,
     *  chunkedWrite: BluetoothRemoteGATTCharacteristic,
     *  chunkedRead: BluetoothRemoteGATTCharacteristic,
     * }}
     */
    this.chars = {};
  }
...
export class MiBand6 extends MiBand {
  /**
   * @param {String} authKey
   *   Hex representation of the auth key (https://codeberg.org/Freeyourgadget/Gadgetbridge/wiki/Huami-Server-Pairing)
   *   Example: '94359d5b8b092e1286a43cfb62ee7923'
...
export class MiBand5 extends MiBand {
  async init() {
    const device = await navigator.bluetooth.requestDevice({
      filters: [
        {
  ...
  async authenticate() {
    await this.startNotifications(this.chars.auth, async (e) => {
      ...
      if (cmd === "100101") {
        ...
        const cmd = this.concatBuffers(new Uint8Array([3, 0]), out);
